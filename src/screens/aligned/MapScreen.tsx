...
const SEED_PINS: MapPin[] = [
  ...
  { id: 2, city: 'Turin', country: 'Italy', lat: 45, lng: 7.7, type: 'home', owner: 'amanda', note: 'Cobblestones, espresso, alpine air.' },
  { id: 3, city: 'New York', country: 'USA', lat: 40.7, lng: -74, type: 'together', dates: 'Dec 2024', note: 'Our first trip together.' },
  { id: 4, city: 'Lake Como', country: 'Italy', lat: 45.9, lng: 9.3, type: 'together', dates: 'Aug 2024', note: 'Long lunches, slow boats.' },
  { id: 5, city: 'Paris', country: 'France', lat: 49, lng: 2, type: 'bucket', note: 'A dream long in the making.' },
  { id: 6, city: 'Tulum', country: 'Mexico', lat: 20, lng: -87, type: 'upcoming', note: 'Spring reunion trip — booked.', dates: 'March 20–27' },
];
      ...
      <ScrollView showsVerticalScrollIndicator={false}>
        <View style={styles.inner}>
          <AppText variant="display" size={42} style={{ lineHeight: 42, marginBottom: 6 }}>
            Map<AppText size={42} color={Colors.accent}>.</AppText>
          </AppText>
          ...
          <View style={styles.statsRow}>
            {[
              ...
              <View key={i} style={[styles.statBlock, i < 3 && styles.statBorder]}>
                <AppText variant="display" size={24} color={Colors.ink} style={{ lineHeight: 24 }}>{s.n}</AppText>
                <AppText variant="smallCaps" color={Colors.muted} style={{ marginTop: 4 }}>{s.l}</AppText>
              </View>
            ))}
          </View>
          ...
          <View style={styles.mapPlaceholder}>
            <AppText variant="serifItalic" size={15} color={Colors.muted} style={{ textAlign: 'center' }}>
              Interactive world map{'\n'}
              <AppText variant="mono" size={10} color={Colors.light}>
                (Integrate react-native-maps or react-native-svg for full implementation)
              </AppText>
            </AppText>
            {/* Legend */}
            <View style={styles.legend}>
              {Object.entries(PIN_META).map(([k, m]) => (
                <View key={k} style={{ flexDirection: 'row', alignItems: 'center', gap: 6 }}>
                  ...
                  <AppText variant="mono" color={Colors.muted} style={{ fontSize: 9 }}>{m.label.toUpperCase()}</AppText>
                </View>
              ))}
            </View>
          </View>
          ...
          <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'baseline', marginBottom: 14 }}>
            <AppText variant="smallCaps" color={Colors.ink2}>{filtered.length} places</AppText>
            <AppButton variant="ghost" size="sm" onPress={() => setSheet('new')}>+ ADD</AppButton>
          </View>
          ...
          {filtered.map(p => (
            <PinCard
              key={p.id}
              pin={p}
              onPress={() => { setSelected(p); setSheet('detail'); }}
            />
          ))}
...
>
  {selected && (
    <>
      <AppText variant="mono" color={Colors.muted} style={{ fontSize: 10, marginBottom: 18 }}>
      ...
      {selected.dates && (
        <View style={styles.dateBlock}>
          <AppText variant="smallCaps" color={Colors.accent} style={{ marginBottom: 4 }}>When</AppText>
          <AppText variant="heading" size={16}>{selected.dates}</AppText>
        </View>
      )}

      {/* ==================== MOMENT SECTION ==================== */}
      <AppText variant="smallCaps" color={Colors.ink2} style={{ marginTop: 28, marginBottom: 12 }}>
        IN THIS MOMENT
      </AppText>
      ...
      <View style={styles.momentGrid}>
        {Moment.map((item, index) => (
          <View key={index} style={styles.momentCard}>
            ...
            <AppText variant="mono" style={{ fontSize: 12, marginTop: 6 }}>
              {item.name}
            </AppText>
          </View>
        ))}
      ...
      {selected.type === 'bucket' && (
        <AppButton
          ...
          onPress={() => {
            const upd = { ...selected, type: 'upcoming' as PinType, dates: 'TBD' };
            setSelected(upd);
            setPins(p => p.map(x => x.id === selected.id ? upd : x));
          }}
        >
          Plan a trip here →
        </AppButton>
      ...
      <AppButton full variant="outline" size="lg" onPress={() => setSheet(null)}>
        Close
      </AppButton>
    </>
  )}
      ...
      {/* Add pin sheet */}
      <BottomSheet open={sheet === 'new'} onClose={() => setSheet(null)} kicker="NEW" title="Mark a place">
        <AppText variant="serifItalic" size={15} color={Colors.muted} style={{ marginBottom: 22 }}>
...
<AppTextInput label="A Note" n="04" placeholder="What Draws you there..." />

        <AppButton
          full variant="solid" size="lg"
          onPress={() => {
            const np: MapPin = {
              id: Date.now(),
